package com.sjy.ttclub.record.datepicker.bizs.calendars;
...
public final class DPCManager {
    ...
    private static final HashMap<String, Set<String>> DECOR_CACHE_SELECTED = new HashMap<>();
    ...
    public void setDecorSelected(List<String> date) {
        if (DECOR_CACHE_SELECTED.size() > 0) {
            DECOR_CACHE_SELECTED.clear();
        }
        setDecor(date, DECOR_CACHE_SELECTED);
    }
...
//        DATE_CACHE.put(year, dataOfYear);
    ...
    private DPInfo[][] buildDPInfo(int year, int month) {
        ...
        Set<String> decorSelected = DECOR_CACHE_SELECTED.get(year + ":" + month);
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                if (c instanceof DPCNCalendar) {
                    if (!TextUtils.isEmpty(tmp.strG))
                        tmp.isSolarTerms = ((DPCNCalendar) c).isSolarTerm(year, month, Integer.valueOf(tmp.strG));
                    if (!TextUtils.isEmpty(strF[i][j]) && strF[i][j].endsWith("F"))
                        tmp.isFestival = true;
                    if (!TextUtils.isEmpty(tmp.strG))
                        tmp.isDeferred = ((DPCNCalendar) c).isDeferred(year, month, Integer.valueOf(tmp.strG));
                } else {
                ...
                if (null != decorSelected && decorSelected.contains(tmp.strG))
                    tmp.isDecorSelected = true;
                info[i][j] = tmp;
            }
        }
        return info;
